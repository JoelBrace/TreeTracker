...
export class FarmingTracker {
    predictPatch(patch, getConfiguration) {
      ...
      const notify_key = "notify."+patch.getRegion().getRegionID() + '.' + patch.getVarbit();
      var notify_storedValue = (target =>
        typeof target === 'function'
          ? target(TimeTrackingConfig.CONFIG_GROUP, notify_key)
          : target.apply(TimeTrackingConfig.CONFIG_GROUP, notify_key))(getConfiguration)
          if (notify_storedValue === null || notify_storedValue === undefined) {
            notify_storedValue = false
          }
          // console.log(notify_key)
          // console.log(notify_storedValue)
      let unixTime = 0
      let value = 0
      {
